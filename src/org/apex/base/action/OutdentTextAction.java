/*
 * OutdentTextAction.java 
 * Created on 25 Nov, 2009, 10:39:55 PM
 *
...
public class OutdentTextAction extends CustomAction {

    /**
     * Creates an instance of {@code OutdentTextAction} with given menu node.
     * @param menu A menu node.
     */
    public OutdentTextAction(MenuNode menu) {
        super(menu);
    }
        ...
        try {
            ...
            if (actualStart >= 0 && actualEnd >= 0) {
                ...
                for (; startRow <= endRow; startRow++) {
                    String beginingText = currentDocument.getDocument().getText(root.getElement(
                            startRow).
                            getStartOffset(), 1);
                    if (beginingText.equalsIgnoreCase("\t")) {
                        currentDocument.getDocument().remove(root.getElement(startRow).
                                getStartOffset(), 1);
                    }
                }
                beep = false;
