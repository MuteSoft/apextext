...
public class CircularCharBuffer {
     ...
     * 
     * @since ostermillerutils 1.00.00
     * @return Number of characters saved for supporting marks.
     */
    private int marked() {
    ...
    protected class CircularCharBufferReader extends Reader {
            ...
            synchronized (CircularCharBuffer.this) {
                if (readerClosed) {
                    throw new IOException(
                            "Reader has been closed; cannot mark a closed Reader.");
                }
                if (buffer.length - 1 <= readAheadLimit) {
                    throw new IOException(
                            "Cannot mark stream, readAheadLimit bigger than buffer size.");
                }
        ...
        public int read() throws IOException {
            while (true) {
                synchronized (CircularCharBuffer.this) {
                    if (readerClosed) {
                        throw new IOException(
                                "Reader has been closed; cannot read from a closed Reader.");
                    }
                    int available = available();
                    if (available > 0) {
        ...
        public void reset() throws IOException {
            synchronized (CircularCharBuffer.this) {
                if (readerClosed) {
                    throw new IOException(
                            "Reader has been closed; cannot reset a closed Reader.");
                }
        ...
        public long skip(long n) throws IOException, IllegalArgumentException {
            while (true) {
                synchronized (CircularCharBuffer.this) {
                    if (readerClosed) {
                        throw new IOException(
                                "Reader has been closed; cannot skip characters on a closed Reader.");
                    }
                    int available = available();
                    if (available > 0) {
    ...
    protected class CircularCharBufferWriter extends Writer {
        ...
        public void flush() throws IOException {
            if (writerClosed) {
                throw new IOException(
                        "Writer has been closed; cannot flush a closed Writer.");
            }
            if (readerClosed) {
